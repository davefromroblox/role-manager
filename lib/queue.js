        ...
        for (const job of jobs) {
            try {
                ...
                if (role.position >= botMember.roles.highest.position) {
                    ...
                    logger.warn(`Job ${job.id}: Bot cannot manage role — hierarchy`);
                    continue;
                }
                ...
                if (job.action_type === 'ADD') {
                    if (!hasRole) await member.roles.add(role, job.reason || undefined);
                ...
                try {
                    ...
                    executorUser = { id: job.actor_id, username: `Unknown (${job.actor_id})` };
                }
